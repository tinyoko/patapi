# 特許庁API & OPD統合 Webアプリケーション

特許庁APIとOPD（ワン・ポータル・ドシエ）APIを使用した特許情報取得・XML書類解析Webアプリケーションです。
...
### OPD（ワン・ポータル・ドシエ）API **新機能**
- **OPD書類一覧取得**: 特許出願に関連する書類一覧を取得
- **JP書類テキスト抽出**: XML形式の書類実体から日本語テキストを抽出
- **対象書類**: 明細書、特許請求の範囲、図面の簡単な説明など
- **構造化テキスト表示**: 抽出されたテキストをセクション別に整理して表示

### 共通機能
- **JSON表示切り替え**: 生データの確認も可能
- **タブ切り替えUI**: 特許経過情報とOPD機能を簡単に切り替え
- エラーハンドリングとローディング表示
